...
def mse(y: np.ndarray, y_hat: np.ndarray) -> float:
    return (1/len(y)) *(np.power(np.linalg.norm(y-y_hat),2))
...
def optimize(
    ...
    y_hat = a * x + b
    errors = y - y_hat
    a_prim = a - learning_rate * (-2 * np.sum(errors *x)/len(errors))
    b_prim = b - learning_rate * (-2 * np.sum(errors)/len(errors))
...
def evaluate_model(
    ...
    with torch.no_grad():
        logits_pred = model(X)
        y_pred_score = sigmoid(logits_pred)
        if not threshold:
            precisions, recalls, thresholds = precision_recall_curve(y.numpy(), y_pred_score.numpy())
...
for epoch_num in range(max_epochs):
    ...
    if valid_metrics["loss"] < best_val_loss:
        ...
        print(f"Early stopping at {epoch_num}")
        break
    else:
...
weights = compute_class_weight(class_weight='balanced', classes=np.unique(y_train.numpy().reshape(-1)), y=y_train.numpy().reshape(-1))
loss_fn = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor(weights[1]/ weights[0]))
...
class OptimizedMLP(nn.Module):
    def __init__(self, input_size: int, dropout_p: float = 0.5, N:int = 256):
        super().__init__()
        self.mlp = nn.Sequential(
...
best_model.eval()
valid_metrics = evaluate_model(
    best_model, X_valid, y_valid, loss_fn, best_threshold
)
...
# %%
study_randome = optuna.create_study(direction='minimize', sampler=optuna.samplers.RandomSampler(seed=0))
study_randome.optimize(objective, n_trials=30)

# %%
for param, val in study_randome.best_params.items():
    print(f"{param}: {val}")
print(study_randome.best_value)

# %%
layer_size =    study_randome.best_params["layer_size"]
learning_rate = study_randome.best_params["learning_rate"]
batch_size = study_randome.best_params["batch_size"]
l2_reg = study_randome.best_params["l2_reg"]
dropout_p = study_randome.best_params["dropout_p"]
model = OptimizedMLP(
    input_size=X_train.shape[1], 
    dropout_p=dropout_p,
    N=layer_size
...
# %% [markdown]
# Z powyższych wyników wynika, że obie metody przeszukiwania chrakteryzują się zbliżonym wartością AUROC. Jednak największą różnicą jest, że model z hiperparametrami znalezionymi przez TPE wyróżnia się większą wartością Recall w porówaniu z Random Search.
